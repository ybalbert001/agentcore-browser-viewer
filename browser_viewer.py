...
from utils import ParameterStoreManager
...
def get_temp_live_view_url(browser_session_id):
    ...
    if not session_data:
        raise ValueError(f"Browser session {browser_session_id} not found in Parameter Store")
...
#debug-info {
    ...
    def _setup_routes(self):
        ...
        async def root(browser_session_id: Optional[str] = Query(None)):
            """Serve the main viewer page."""
            try:
                # Determine which URL to use
                if browser_session_id:
                    # Get live_view_url from Parameter Store using browser_session_id
                    try:
                        self.presigned_url = get_temp_live_view_url(browser_session_id)
                        if not self.presigned_url:
                            raise HTTPException(status_code=404, detail=f"Live view URL not found for session {browser_session_id}")

                        console.print(f"\n[cyan]Using live view URL from session {browser_session_id}:[/cyan]")
                        console.print(f"[dim]{self.presigned_url}[/dim]\n")

                        html = self._generate_html(self.presigned_url)
                        return HTMLResponse(content=html)
                    except ValueError as e:
                        console.print(f"[red]Error getting session data: {str(e)}[/red]")
        ...
        async def debug_info():
            ...
            return {
                ...
                "session": {
                    "presigned_url": self.presigned_url[:100] + "..." if len(self.presigned_url) > 100 else self.presigned_url
                },
                "server": {
...
<body>
    <div class="container">
        <div class="header">
            <h2>Bedrock-AgentCore Browser Viewer</h2>
        </div>
    ...
    def start(self, browser_session_id:str, open_browser: bool = True) -> str:
        ...
        viewer_url = f"http://localhost:{self.port}/?browser_session_id={browser_session_id}"
